...
# Read in data
accounts = pd.read_csv("Resources/accounts.csv") #P1
gpa = pd.read_csv("Resources/group_product_associations.csv") #P1
ast = pd.read_csv("Resources/account_state_transitions.csv") #P2 & P4
payments = pd.read_csv("Resources/payments.csv") #P3
accounts = pd.read_csv("Resources/accounts.csv") #P4
groups = pd.read_csv("Resources/groups.csv") #P4


### PART 1: VISUALIZE PRODUCT REGISTRATION ###
# Rename columns
...
### ADJUST DATA TYPE AND LIMIT DATAFRAME FOR PARTS 2-4 ###
# Change data type from string to datetime object
ast['started_when'] = pd.to_datetime(ast['started_when']) #P2
payments['effective_when'] = pd.to_datetime(payments['effective_when']) #P3
aag_merge['started_when'] = pd.to_datetime(aag_merge['started_when']) # P4

# Create limited dataframe based on indicated timestamp 
ast_date_limited = ast.loc[ast['started_when'] <= timestamp_dt] #P2
payments_date_limited = payments.loc[payments['effective_when'] <= timestamp_dt] #P3

# Also limit by 'to_state'
aag_merge_date_limited = aag_merge.loc[(aag_merge['started_when'] <= timestamp_dt) & (aag_merge['to_state']=='DISABLED')] #P4


### PART 2: TOTAL TIME IN STATE ###
...
### PART 4: NOMINAL EXPECTED PAYMENTS ###
...
ast_filtered.to_csv(f"Output/time_in_state_as_of_{timestamp_str[:10]}.csv", index=False) #P2
cumulative_payments.to_csv(f"Output/cumulative_payments_by_account_as_of_{timestamp_str[:10]}.csv", index=False) #P3
expected_payments_df.to_csv(f"Output/nominal_expected_payments_as_of_{timestamp_str[:10]}.csv", index=False) # P4